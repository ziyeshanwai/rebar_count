...
class Network(object):
  ...
  def _crop_pool_layer(self, bottom, rois, name):
    with tf.variable_scope(name) as scope:
      ...
      x1 = tf.slice(rois, [0, 1], [-1, 1], name="x1") / width  # 这里归一化
  ...
  def _build_network(self, is_training=True):
    ...
    feature_map = self._get_feature_map() # 获取feature_map
    with tf.variable_scope(self._scope, self._scope):
      # build the anchors for the image
      self._anchor_component()
      # region proposal network
      rois = self._region_proposal(net_conv, is_training, initializer)  # 在net_conv上proposal 在 feature_map上提取特征
      # region of interest pooling
      if cfg.POOLING_MODE == 'crop':
        # pool5 = self._crop_pool_layer(net_conv, rois, "pool5")  # roi pooling
        pool5 = self._crop_pool_layer(feature_map, rois, "pool5")  # roi pooling
